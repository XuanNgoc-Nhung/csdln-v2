...
$('#btn-themTieu-Chi').on('click', function () {
    ...
    var mo_ta = $("#moTa").val();
    var id_tieu_chi = $('#idTieuChi').val();
    var maTieuChi = $('#maTieuChi').val();
    if (!ten_tieu_chi) {
    ...
    if (doi_tuong_ap_dung != 'Custom' && !phuong_phap) {
    ...
    console.error("Ấn thêm mới:")
    console.log(thongTinTieuChi)
    ...
    var params = {
        ...
        moTa: mo_ta,
        id: id_tieu_chi,
        maTieuChi: thongTinTieuChi.maTieuChi,
        commonUsed: thongTinTieuChi.commonUsed,
        formula: thongTinTieuChi.formula,
        filters: thongTinTieuChi.filters,
        generatedMongoQuery: thongTinTieuChi.generatedMongoQuery,
        relatedIndicators: thongTinTieuChi.relatedIndicators,
        userNameCreate: thongTinTieuChi.userNameCreate,
    }
    ...
    $.confirm({
        title: 'Xác nhận chỉnh sửa thông tin tiêu chí?',
        content: '',
        buttons: {
            Huy: {
                text: 'Hủy',
...
function themTieuChi(e) {
    ...
    $.ajax({
        ...
        type: 'POST',
        url: '/api-post-chinh-sua-tieu-chi',
        data: e,
        dataType: 'html',
        ...
        if (kq.rc == 0) {
            notif({
                msg: "Chỉnh sửa tiêu chí thành công",
                type: "success"
            });
            $('.show_add').css({
                "display": "none",
