...
$('#btn-themTieu-Chi').on('click', function () {
    ...
    if (doi_tuong_ap_dung!='Custom'&&!phuong_phap) {
        console.warn("doi_tuong_ap_dung")
        console.log(doi_tuong_ap_dung)
        console.log(phuong_phap)
    ...
    $.confirm({
        title: 'Xác nhận thêm mới tiêu chí?',
        content: '',
        buttons: {
            Huy: {
                text: 'Hủy',
...
function themTieuChi(e) {
    ...
    $.ajax({
        ...
        if (kq.rc == 0) {
            notif({
                msg: "Thêm mới tiêu chí thành công",
                type: "success"
            });
            console.log(kq.item.maTieuChi)
            $('#maTieuChiClone').text(kq.item.maTieuChi)
            $('.show_add').css({
                "display": "none",
            });
            $('.show_done').css({
