<template>
  <div id="modal_chinhsua_monhoc" ref="modal_chinhsua_monhoc" class="modal fade">
    <div class="modal-dialog" style="width:80%">
      <div class="modal-content">
        <form @submit.prevent="suaMonHocSubmit">
          <div class="modal-header">
            <button
              ...
              v-on:click="close()"
            >&times;</button>
            <h4 class="modal-title text-uppercase">Chỉnh sửa môn học</h4>
          </div>
            ...
            <div style="padding-top: 10px">
              <div class="form-group clearfix">
                ...
                <div class="col-sm-3 inputhemmon">
                  ...
                  <span
                    class="text-danger"
                    v-show="errors.has('loai_danh_gia')"
                  >{{ errors.first('loai_danh_gia') }}</span>
                </div>
                <div class="col-sm-3 inputhemmon">
                ...
                <div class="col-sm-3">
                  ...
                  <div style="padding-top:7px;">
                    <label style="padding:0 25px; font-weight:100;">
                      <input v-model="capmot" type="checkbox" value />Cấp
                      1
                    </label>
                    <label style="padding:0 25px; font-weight:100;">
                      <input v-model="caphai" type="checkbox" value />Cấp
                      2
                    </label>
                    <label style="padding:0 25px; font-weight:100;">
                      <input v-model="capba" type="checkbox" value />Cấp 3
                    </label>
                  </div>
                </div>
                <div class="col-md-3 inputhemmon">
                  <label>Tính chất môn</label> <br>
                  <div style="width:50%; float:left;font-weight:100;">
...
export default {
  ...
  watch: {
    show: function(newVal, oldVal) {
      ...
      if (newVal) {
        this.reset();
        $("#modal_chinhsua_monhoc").modal();
      } else {
        $("#modal_chinhsua_monhoc").modal("hide");
      }
    },
    item: function(newVal, oldVal) {
      this.ma_monhoc = newVal.maMonHoc;
      this.ten_monhoc = newVal.tenMonHoc;
      this.ten_viettat = newVal.tenVietTat;
      this.tieng_dan_toc = newVal.tiengDanToc;
      this.mon_ngoai_ngu = newVal.ngoaiNgu;
      this.mon_nghe = newVal.monNghe;
      this.nhom_nghe = newVal.nhomNghe;
      this.trangThai = newVal.trangThai;
      if (this.mon_nghe == 0) {
        this.nhom_nghe == null;
        this.nhom_nghe = "";
      }
      this.loai_danh_gia = newVal.loaiDanhGia;
      this.mien_giam = newVal.mienGiam;
      this.thuc_hanh = newVal.thucHanh;
      this.caphoc = newVal.capHoc;
      this.list_caphoc = Array.from(this.caphoc);
      this.capmot = "";
      this.caphai = "";
      this.capba = "";
      for (var i = 0; i < this.list_caphoc.length; i++) {
        if (this.list_caphoc[i] == 1) {
          this.capmot = true;
        }
        if (this.list_caphoc[i] == 2) {
          this.caphai = true;
        }
        if (this.list_caphoc[i] == 3) {
          this.capba = true;
        }
      }
    }
  ...
  mounted: function() {
    console.log("mounted !!!!!");
    $(this.$refs.modal_chinhsua_monhoc).on("hidden.bs.modal", this.close);
  },
  ...
  methods: {
    ...
    chinhSuaMonHoc() {
      ...
      utils.confirmDialog("Xác nhận chỉnh sửa môn học", "Sửa", () => {
        const uri = "/api_chinh_sua_mon_hoc";
        const params = {
