<template>
    ...
    <div id="modal_them_doituongchinhsach" ref="modal_them_doituongchinhsach" class="modal fade">

        <div class="modal-dialog">

            <div class="modal-content">
                <form @submit.prevent="themDoiTuongChinhSachSubmit">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true" v-on:click="close()">
                            &times;
                        </button>
                        <h4 class="modal-title text-uppercase">Thêm đối tượng chính sách</h4>
                    </div>
    ...
    export default {
        ...
        watch: {
            show: function (newVal, oldVal) {
                ...
                if (newVal) {
                    this.reset();
                    $("#modal_them_doituongchinhsach").modal();
                } else {
                    $("#modal_them_doituongchinhsach").modal('hide');
                }
            }
        },
        ...
        mounted: function () {
            console.log("mounted !!!!!");
            $(this.$refs.modal_them_doituongchinhsach).on("hidden.bs.modal", this.close);
            this.customMessage();
        },
        ...
        methods: {
            customMessage(){
                const dict = {
                    custom: {
                        ma_doituong: {
                            required: () => 'Mã đối tượng chính sách không thể trống',
                            max: () => 'Mã đối tượng chính sách không vượt quá 30 ký tự',
                            regex: () => 'Mã đối tượng chính sách chỉ chứa chữ cái, chữ số, dấu gạch dưới'
                        },
                        ten_doituong: {
            ...
            createNation() {
                utils.confirmDialog("Xác nhận thêm đối tượng chính sách", "Thêm", () => {
                    const uri = "/api_them_doi_tuong_chinh_sach";
                    const params = {
                    ...
                    api.post(uri, params, (response) => {
                        ...
                        if (response.data.code === 200) {
                            utils.showDialog("Thông báo", "Thêm đối tượng chính sách thành công", null, () => {
