<template>
    ...
    <div id="modal_them_chiso" ref="modal_them_chiso" class="modal fade">

        <div class="modal-dialog">

            <div class="modal-content">
                <form @submit.prevent="themChiSoDanhGiaPhatTrienSubmit">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true" v-on:click="close()">
                            &times;
                        </button>
                        <h4 class="modal-title text-uppercase">Thêm chỉ số đánh giá phát triển</h4>
                    </div>
                    <div class="modal-body clearfix">
                        ...
                        <div style="padding-top: 10px">
                            <div class="form-group clearfix">
                                ...
                                <div class="col-sm-8">
                                    <input v-validate="{ required: true, max:30, regex: /^[a-zA-Z0-9_]*$/}" name="ma_chiso" type="text"
                                           min="0"
                                           v-model="ma_chiso"
                                           placeholder="Nhập..."
                                           class="form-control control-inline"
                                           style="min-width: 50px; max-width: 250px"/>
                                    <span v-if="show_msg"><span class="text-danger" v-show="errors.has('ma_chiso')">{{ errors.first('ma_chiso') }}</span></span>
                                </div>
                            </div>
                        </div>
                        <div style="padding-top: 10px">
                            <div class="form-group clearfix">
                                ...
                                <div class="col-sm-8">
                                    <input v-validate="{ required: true}" name="ten_chiso" type="text"
                                           ...
                                           class="form-control control-inline"
                                           style="min-width: 50px; max-width: 250px"/>
                                    <span v-if="show_msg"><span class="text-danger" v-show="errors.has('ten_chiso')">{{ errors.first('ten_chiso') }}</span></span>
                                </div>
                            </div>
                        </div>
                        <div style="padding-top: 10px">
    ...
    export default {
        ...
        watch: {
            show: function (newVal, oldVal) {
                ...
                if (newVal) {
                    this.reset();
                    $("#modal_them_chiso").modal();
                } else {
                    $("#modal_them_chiso").modal('hide');
                }
            }
        },
        ...
        mounted: function () {
            console.log("mounted !!!!!");
            $(this.$refs.modal_them_chiso).on("hidden.bs.modal", this.close);
        ...
        methods: {
            customMessage(){
                const dict = {
                    custom: {
                        ma_chiso : {
                            required: () => 'Mã chỉ số không thể trống',
                            max: () => 'Mã chỉ số không vượt quá 30 ký tự',
                            regex: () => 'Mã chỉ số chỉ chứa chữ cái, chữ số, dấu gạch dưới'
                        },
                        ma_linhvuc : {
            ...
            themChiSoDanhGiaPhatTrienSubmit(){
                this.show_msg = true;
            ...
            createChiSo() {
                console.log("Thêm chỉ số");
                utils.confirmDialog("Xác nhận thêm chỉ số", "Thêm", () => {
                    const uri = "/api_them_chi_so_danh_gia_phat_trien";
                    const params = {
                    ...
                    console.log("các param");
                    api.post(uri, params, (response) => {
                        console.log(response);
                        if (response.data.code === 200) {
                            utils.showDialog("Thông báo", "Thêm chỉ số thành công", null, () => {
                                this.$emit('success');
                                this.show_msg = false;
                                this.reset();
                                this.close();
