<template>
    <div class="row" v-if="total_rows>0" style="padding-bottom:150px;">
        <div class="col-sm-3">
        ...
        <div class="col-sm-9">
            <b-pagination class="pull-right" :first-text="pagination.first_text" :limit="9"
                :last-text="pagination.last_text" :total-rows="total_rows" :prev-text="pagination.next_text"
                :next-text="pagination.prev_text" v-model="currentPage" @input="chonTrang" :per-page="pagination.limit"></b-pagination>

            <div class="vanban">
                <p><i>Hiển thị từ <span style="color:#478fca">{{start}} - {{to}}</span> /Tổng số <b>{{formatNumber(total_rows)}}</b> bản ghi</i></p>
            </div>
        </div>
    </div>
</template>
