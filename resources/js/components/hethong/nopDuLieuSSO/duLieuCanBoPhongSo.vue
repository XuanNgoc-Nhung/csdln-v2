    ...
    <div v-loading.fullscreen.lock="loading.status" :element-loading-text="loading.text"
        ...
        <el-row :span="20">
            <el-col :xs="24" :sm="12" :md="8" :lg="6">
                ...
                <eselect multiple collapseTags v-model="dataSearch.donVis" :placeholder="'Chọn'" filterable :data="list_don_vi"
                         :fields="['name','value']"/>
            </el-col>
            <el-col :xs="24" :sm="12" :md="8" :lg="6">
                <label>Năm học:</label>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <tr style="background:#e4ebf5">
                            ...
                            <th class="text-center"><p>Chức vụ</p></th>
                        ...
                        <tr v-bind:class="{ mauDo: item.syncMaSoGd!=thongtin.ma_so }" v-for="(item,i) in list_can_bo"
                            ...
                            <td class="text-center"><p>{{ item.maCanBo }}</p></td>
                            <td><p>{{ item.tenCanBo }}</p></td>
                            <td class="text-center"><p>{{ parseInt(item.gioiTinh) === 1 ? 'Nam' : 'Nữ' }}</p></td>
                            <td><p>{{ item.tenChucVu }}</p></td>
                            <td class="text-center"><p>{{ item.tenHuyen }}</p></td>
                            <td><p>{{ item.tenTinh }}</p></td>
                            ...
                            <td class="text-center">
                                <input type="checkbox"
                                       ...
                                       v-model="list_can_bo_checked" :value="item.maCanBo"/>
        ...
        <el-row :gutter="20">
            <el-col :span="24">
                <ImportFile v-if="show_import" @close="show_import = false"
                            :text_header="'Import dữ liệu cán bộ phòng sở'"
                            :nam_hoc="dataSearch.namHoc"
                            :sample_file_path="fileMau"
                            @success='importSuccess'
                            :path_import="'/api-import-du-lieu-sso-can-bo-phong-so'"></ImportFile>
...
import ChinhSua from './chinhSuaCanBoPhongSo'
...
export default {
    ...
    data() {
        return {
            fileMau: 'https://dongbo.csdl.edu.vn/exportExcel/FileUpLoadDuLieuCanbo.xlsx',
            loading: {
                status: false,
                text: 'Loading...'
            },
    ...
    methods: {
        XoaDuLieu() {
            if (this.list_can_bo_checked.length == 0) {
                this.thongBao('warning', 'Vui lòng chọn cán bộ cần xoá');
            } else {
                this.$confirm('Xác nhận xoá thông tin các cán bộ đã chọn?', 'Thông báo', {
                    ...
                    .then(_ => {
                        ...
                        rest_api.post("/csdlgd-admin/sso/deleteCanBoPhongSo", params, response => {
        ...
        chonTatCa() {
            if (this.checkAll) {
                ...
                if (countSelected == 0) {//Không chọn được bản ghi nào
                    this.thongBao('error', 'Không có cán bộ nào trực thuộc đơn vị của bạn');
                    this.checkAll = false;
                    return;
                } else {
                    if (countUnSelected > 0) {//Chọn được nhưng không hết
                        let msg = 'Chọn được ' + countSelected + '/' + this.list_can_bo.length + ' cán bộ. ' + countUnSelected + ' cán bộ còn lại không trực thuộc đơn vị quản lý của bạn.'
                        this.thongBao('warning', msg);
                        this.checkAll = false;
                    } else {//Chọn được tất cả
                        this.thongBao('success', 'Đã chọn toàn bộ ' + this.list_can_bo.length + ' cán bộ')
                    }
                }
            } else {
        ...
        checkExportSuccess(e) {
            ...
            let params = {
                type: 3,//Dữ liệu cán bộ phòng sở
                start: e.start,
                limit: e.limit,
        ...
        getData() {
            ...
            try {
                ...
                this.loading.status = true;
                rest_api.post("/csdlgd-admin/sso/canBoPhongSo", params, response => {
                    console.log("Dữ liệu cán bộ:")
                    console.log(response)
                    if (response && response && response.data.danhSachCanBoPhongSo) {
                        this.list_can_bo = response.data.danhSachCanBoPhongSo;
