<template>
    <div id="modal_them_tieu_chi" ref="modal_them_tieu_chi" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true" v-on:click="close()">
                        &times;
                    </button>
                    <h4 class="modal-title text-uppercase">THÊM MỚI TIÊU CHÍ</h4>
                </div>
                <div class="modal-body clearfix">
                    <div style="padding-top: 10px">
                        <div class="form-group clearfix">
                            <div class="col-sm-4">
                                <label class="pull-right" style="line-height: 34px;">Tên tiêu chí *</label>
                            </div>
                            <div class="col-sm-8">
                                <input type="text"
                                       min="0"
                                       v-model="ten_tieu_chi"
                                       placeholder="Nhập..."
                                       class="form-control control-inline"
                                       style="min-width: 50px; max-width: 250px"/>
                            </div>
                        </div>
                    </div>
                    <div style="padding-top: 10px">
                        <div class="form-group clearfix">
                            <div class="col-sm-4">
                                <label class="pull-right" style="line-height: 34px;">Mã tiêu chí *</label>
                            </div>
                            <div class="col-sm-8">
                                <input type="text"
                                       min="0"
                                       v-model="ma_tieu_chi"
                                       placeholder="Nhập..."
                                       class="form-control control-inline"
                                       style="min-width: 50px; max-width: 250px"/>
                            </div>
                        </div>
                    </div>

                    <div style="padding-top: 10px">
                        <div class="form-group clearfix">
                            <div class="col-sm-4">
                                <label class="pull-right" style="line-height: 34px;">Tiêu chí cha</label>
                            </div>
                            <div class="col-sm-8">
                                <select v-if="tieu_chi_cha == null"
                                        class="form-control"
                                        disabled
                                        style="min-width: 50px; max-width: 250px"
                                        v-model="tieu_chi_cha_id">
                                    <option selected = "selected" value="">-- Là tiêu chí cha --</option>
                                </select>
                                <select v-else
                                        class="form-control"
                                        disabled
                                        style="min-width: 50px; max-width: 250px">
                                    <option selected="selected" value="">{{this.ten_tieu_chi_cha}}</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div style="padding-top: 10px">
                        <div class="form-group clearfix">
                            <div class="col-sm-4">
                                <label class="pull-right" style="line-height: 34px;">Trạng thái</label>
                            </div>
                            <div class="col-sm-8">
                                <select
                                        class="form-control ng-pristine ng-valid ng-empty text-center ng-valid-maxlength ng-touched"
                                        style="min-width: 50px; max-width: 250px"
                                        v-model="trang_thai">
                                    <option value="0">Đang hoạt động</option>
                                    <option value="1">Hết hiệu lực</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div style="padding-top: 10px">
                        <div class="form-group clearfix">
                            <div class="col-sm-4">
                                <label class="pull-right" style="line-height: 34px;">Nhóm tiêu chí *</label>
                            </div>
                            <div class="col-sm-8">

                                <select v-if="tieu_chi_cha == null"
                                        class="form-control ng-pristine ng-valid ng-empty text-center ng-valid-maxlength ng-touched"
                                        style="min-width: 50px; max-width: 250px"
                                        v-model="nhom_tieu_chi_id">
                                    <option selected="selected" value="">-- Chọn --</option>

                                    <template v-for="ds in ds_tieu_chi" :label="nhomtieuchi">

                                        <option v-if="!ds.listChild[0]" :value="ds.nhomTieuChiId">
                                            {{ds.tenNhomTieuChi}}
                                        </option>
                                        <optgroup v-if="ds.listChild[0]" :label="ds.tenNhomTieuChi">
                                            <option v-for="option in ds.listChild" :value="option.nhomTieuChiId">
                                                {{option.tenNhomTieuChi}}
                                            </option>
                                        </optgroup>
                                    </template>



                                </select>
                                <select v-else
                                        class="form-control ng-pristine ng-valid ng-empty text-center ng-valid-maxlength ng-touched"
                                        style="min-width: 50px; max-width: 250px"
                                        disabled>
                                    <option selected="selected" value="">
                                        {{this.ten_nhom_tieu_chi}}
                                    </option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div style="padding-top: 10px">
                        <div class="form-group clearfix">
                            <div class="col-sm-4">
                                <label class="pull-right" style="line-height: 34px;">Loại tiêu chí</label>
                            </div>
                            <div class="col-sm-8">
                                <select
                                        class="form-control ng-pristine ng-valid ng-empty text-center ng-valid-maxlength ng-touched"
                                        style="min-width: 50px; max-width: 250px"
                                        v-model="loai_tieu_chi">
                                    <option value="0">Storeproc</option>
                                    <option value="1">Script</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div style="padding-top: 10px">
                        <div class="form-group clearfix">
                            <div class="col-sm-4">
                                <label class="pull-right" style="line-height: 34px;">Script *</label>
                            </div>
                            <div class="col-sm-8">
                                <input type="text"
                                       min="0"
                                       v-model="script"
                                       placeholder="Nhập..."
                                       class="form-control"
                                       style="min-width: 50px; max-width: 250px; min-height: 100px;max-height: 150px"/>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer clearfix">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Hủy
                    </button>
                    <button type="button" class="btn btn-primary" v-on:click="createTieuChi">Thêm
                    </button>
                </div>
            </div>

        </div>
    </div>
</template>

<script>

    import moment from 'moment/moment'
    import utils from '../../utils'
    import rest_api from '../../utils/rest_api';

    $(document).ready(function () {
        // console.log("ready!");

    });

    export default {

        props: ['item', 'show', 'tieuchi'],


        watch: {
            show: function (newVal, oldVal) {
                // console.log("newVal = " + newVal);
                if (newVal) {
                    this.reset();
                    $("#modal_them_tieu_chi").modal();
                } else {
                    $("#modal_them_tieu_chi").modal('hide');
                }
            },
            item: function (newVal, oldVal) {
                if (newVal != null) {
                    this.tieu_chi_cha = newVal;
                    this.tieu_chi_cha_id = newVal.tieuChiBaoCaoId;
                    this.nhom_tieu_chi_id = newVal.nhomTieuChiId;
                    this.ten_tieu_chi_cha = newVal.tenTieuChi;
                    this.ten_nhom_tieu_chi = newVal.tenNhomTieuChi;
                    // console.log("ten tieu chi cha: " + this.ten_tieu_chi_cha);
                    // console.log("tiêu chí cha không null");
                } else {
                    this.tieu_chi_cha = null;
                    console.log("tiêu chí cha null");
                }
            },
            tieuchi: function (newVal, oldVal) {
                // console.log("   9999999999999999 tc: " + JSON.stringify(newVal));
                this.ds_tieu_chi = newVal;
                // for (let i = 0; i < newVal.length; i++) {
                //     let obj = newVal[i];
                //     if (parseInt(obj.capNhom) === 0) {
                //         if (obj.listChild != null && obj.listChild.length > 0) {
                //             for (let j = 0; j < obj.listChild.length; j++) {
                //                 let child = obj.listChild[j];
                //                 this.ds_tieu_chi_con.push(child)
                //             }
                //         }
                //     }
                // }
                console.log("ds tiêu chí 1231231313: " + JSON.stringify(this.ds_tieu_chi));
            }
        },

        data() {
            return {
                ma_tieu_chi: '',
                ten_tieu_chi: '',
                tieu_chi_cha_id: '',
                trang_thai: '0',
                nhom_tieu_chi_id: '',
                loai_tieu_chi: '1',
                script: '',


                ds_tieu_chi: [],
                tieu_chi_cha: {},
                ten_tieu_chi_cha: '',
                ten_nhom_tieu_chi: '',


            }
        },
        mounted: function () {

            $(this.$refs.modal_them_tieu_chi).on("hidden.bs.modal", this.close);
        },

        methods: {
            createTieuChi() {
                if (this.ten_tieu_chi == '' || this.ma_tieu_chi == '' || this.nhom_tieu_chi_id == '' || this.script == '') {
                    utils.showDialog("Thông báo", "Bạn chưa nhập đủ thông tin.");
                    return;
                }
                utils.confirmDialog("Xác nhận thêm tiêu chí", "Thêm", () => {
                    let uri = "/csdlgd-admin/bcdTieuChi";
                    let params = {
                        // "limit": 0,
                        "loaiTieuChi": this.loai_tieu_chi,
                        "maTieuChi": this.ma_tieu_chi,
                        "nhomTieuChiId": this.nhom_tieu_chi_id,
                        // "phanCap": "string",
                        "scriptDongBo": this.script,
                        // "start": 0,
                        "tenTieuChi": this.ten_tieu_chi,
                        // "tieuChiBaoCaoId": 0,
                        "tieuChiChaId": this.tieu_chi_cha_id,
                        "trangThai": this.trang_thai,
                    };
                    rest_api.post(uri, params, (response) => {

                        if (response.data.rc === 0) {
                            utils.showDialog("Thông báo", "Thêm tiêu chí thành công", null, () => {
                                this.$emit('success', params);
                                this.close();
                            });
                        } else {
                            utils.showDialog("Thất bại", response.data.message, null, () => {
                            });
                        }
                        console.log('Res Them tieu chi:' + JSON.stringify(response))
                    });
                })
            },
            reset() {
                this.ten_tieu_chi = "";
                this.ma_tieu_chi = "";
                this.script = "";
                // this.tieu_chi_cha = null;
                this.tieu_chi_cha_id = '';
                this.nhom_tieu_chi_id = '';
                // this.ds_tieu_chi = [];
                this.trang_thai = "0";
                this.loai_tieu_chi = "1";
            },

            close() {
                // console.log("close modal");
                this.reset();
                this.$emit('close');
            },

        }

    }
</script>

