                ...
                <div class="modal-content modal-content-fullscreen" style="">
                    ...
                    <div class="modal-body modal-body-fullscreen" style="padding-bottom:200px;">
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <label for="">Nhóm điều kiện</label>
                    ...
                    <div class="modal-footer modal-footer-fullscreen" style="">
                        ...
                        <button class="btn btn-primary" @click.prevent="update()">Thêm</button>
    ...
    export default {
        ...
        methods: {
            update() {
                ...
                if (!this.query.children||this.query.children=='undefined'||this.query.children.length==0) {
                    this
                        .$alertify
                        .error("Vui lòng bổ sung nhóm điều kiện");
                    return;
                    ...
                    if (this.query.children[i].type == "query-builder-group") {
                        for (let j = 0; j < this.query.children[i].query.children.length; j++) {
                            if (this.query.children[i].query.children[j].type == "query-builder-rule") {
                                this.query.children[i].query.children[j].rule = this.query.children[i].query.children[j]
                                    .query.rule;
                                this.query.children[i].query.children[j].operator = this.query.children[i].query
                                    .children[j].query.operator;
                                this.query.children[i].query.children[j].value = this.query.children[i].query.children[
                                    j].query.value;
                                this.query.children[i].query.children[j].type = this.query.children[i].query.children[j]
                                    .query.type;
                            ...
                            if (this.query.children[i].query.children[j].type == "query-builder-group") {
                                for (let k = 0; k < this.query.children[i].query.children[j].query.children
                                    ...
                                    if (this.query.children[i].query.children[j].query.children[k].type ==
                                        ...
                                        this.query.children[i].query.children[j].query.children[k].operator = this.query
                                            .children[i].query
                                            .children[j].query.children[k].query.operator;
                                        this.query.children[i].query.children[j].query.children[k].value = this.query
                                            .children[i].query
                ...
                rest_api.post("/csdlgd-analytic-api/criteria/create", params, data => {
                    if (data.data.rc == 0) {
                        this
                            .$alertify
                            .success("Bạn đã tạo thành công tiêu chí " + data.data.item.tenTieuChi +
                                " với mã tiêu chí " + data.data.item.maTieuChi);
                        this.$emit('success')
                    } else {
            ...
            getChoices(e) {
                ...
                // Hết cán bộ giáo viên, sang lớp học
                if (e == "DM_SO_BUOI_HOC_TREN_TUAN") {
                    choice = JSON.parse(localStorage["danh_sach_so_buoi_hoc_tren_tuan_sme"])
                }
                if (e == "DM_LOP_DAO_TAO_BOI_DUONG") {
                    choice = JSON.parse(localStorage["danh_sach_lop_dao_tao_boi_duong_sme"])
                }
                if (e == "DM_NHOM_TUOI_MN") {
                    choice = JSON.parse(localStorage["danh_sach_nhom_tuoi_mam_non_sme"])
                }
                if (e == "DM_BO_SACH_GIAO_KHOA_SU_DUNG") {
                    choice = JSON.parse(localStorage["danh_sach_bo_sach_giao_khoa_dang_dung"])
                }
                // Trường học, điểm trường
                if (e == "DM_TRUNG_TAM") {
                    choice = JSON.parse(localStorage["danh_sach_danh_muc_trung_tam"])
                }
                if (e == "DM_LOAI_TRUONG") {
                    choice = JSON.parse(localStorage["danh_sach_danh_muc_loai_truong"])
                }
                if (e == "DM_VUNG") {
                    choice = JSON.parse(localStorage["danh_sach_danh_muc_vung"])
                }
                if (e == "DM_LOAI_HINH_DAO_TAO") {
                    choice = JSON.parse(localStorage["danh_sach_loai_hinh_dao_tao_sme"])
                }
                if (e == "DM_NHOM_CAP_HOC") {
                    choice = JSON.parse(localStorage["danh_sach_cap_hoc_2_sme"])
                }
                if (e == "DM_LOAI_TRUNG_TAM") {
                    choice = JSON.parse(localStorage["danh_sach_danh_muc_loai_trung_tam"])
                }
                if (e == "DM_DU_AN") {
                    choice = JSON.parse(localStorage["danh_sach_danh_muc_du_an"])
                }
                if (e == "DM_LOAI_HINH_TRUONG") {
                    choice = JSON.parse(localStorage["danh_sach_loai_hinh_truong_sme"])
                }
                if (e == "DM_MUC_DAT_CHUAN_QG_CLGD") {
                    choice = JSON.parse(localStorage["danh_sach_chuan_quoc_gia_sme"])
                }
                if (e == "DM_VUNG_KHO_KHAN") {
                    choice = JSON.parse(localStorage["danh_sach_doi_tuong_uu_tien_sme"])
                }
            ...
            ChonDoiTuong() {
                ...
                rest_api.get("/csdlgd-admin/sme/getSmeTheoDoiTuong", params, data => {
                    console.log("Lấy sme theo đối tượng:");
